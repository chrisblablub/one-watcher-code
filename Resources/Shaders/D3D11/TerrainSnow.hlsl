...
PSOutput PS(VSOutputTerrain input)
{
    ...
    /*if (input.painterAlpha > 0.01)
    {
        ...
        float2 firstTexCoords = input.texVertex.xy * gTerrainPainterFirstScale.xy;

        float3x3 tangentToWorld = TBN;
        float3 normalNMap = t1.Sample(s1, firstTexCoords).xyz;
        normalNMap = pow(normalNMap, float3(0.4545, 0.4545, 0.4545));
        normalNMap = 2.0f*normalNMap - float3(1.0f, 1.0f, 1.0f);
        normalNMap = mul(normalNMap, tangentToWorld).xyz;

        normal = normalize(lerp(normal, normalNMap, input.painterAlpha * 0.6));
    }*/
    ...
    if (gHasPrecomputedLighting)
    {
        ...
        emissive.rgb += 1.2 * sampledColor.rgb* input.colorLighting;
    }
    ...
    // Add snow
    float snow = max(dot(normalize(normal + float3(0, 0.1, 0)), normalize(float3(0, 1, -0.07))), 0.0) * 1.5;
    snow = snow * snow * snow;

    diff.rgb = lerp(diff.rgb, input.color, clamp(snow * (1.0f - input.texVertex.z / 300.0f), 0, 1));
    diff.rgb = lerp(diff.rgb, float3(0.7, 0.8, 1.0), 0.012);

    PSOutput outPS;
    outPS.RT1 = float4(diff.rgba);
    outPS.RT2 = float4(normal, 0.2 * dot(input.color, float3(0.2125f, 0.7154f, 0.0721f)));
